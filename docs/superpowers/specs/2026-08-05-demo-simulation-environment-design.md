...
| 4 | **Menus depend on Google Fonts at render time** ⚠️ | Theme shell preconnects to `fonts.googleapis.com` / `fonts.gstatic.com` and loads Cormorant Garamond + Inter over the wire (`_shared/shell.ts:23`, `_shared/brand.ts:15`). The iframe is `allow-same-origin` specifically to permit this (`page.tsx:249`), and the theme CSP pins `font-src` to `fonts.gstatic.com` only (`brand.ts:20`) — so a self-hosted fallback is not merely absent, it is currently *forbidden*. | Inline the two families as base64 woff2 in the shell and widen `font-src` to `data:`. Today a venue wifi outage silently drops menus to system fallback fonts — a real hole in the offline story we are about to sell. |
