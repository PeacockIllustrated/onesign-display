 ...
 *
 * Icon draws with glowing stroke, then DISPLAY letters draw in with stagger.
 * Shows every refresh while testing.
 */
...
export default function SplashScreen() {
  ...
  useEffect(() => {
    ...
    const t1 = setTimeout(() => setPhase('text'), 1600)      // icon done, start text
    const t2 = setTimeout(() => setPhase('visible'), 3000)    // text done, tagline
    const t3 = setTimeout(() => setPhase('exiting'), 3800)    // start fade out
    const t4 = setTimeout(() => setPhase('gone'), 4500)       // remove
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: phase === 'exiting' ? 'opacity 700ms cubic-bezier(0.4, 0, 0.2, 1)' : 'none',
      }}
    >
      <style>{`
      ...
      <div
        style={{
          animation: iconDone ? 'splashGlowPulse 2.5s ease-in-out infinite' : 'none',
      ...
      >
        <svg
          ...
          style={{ width: 120, height: 'auto', position: 'absolute', inset: 0 }}
        >
          <defs>
            <filter id="splash-icon-glow">
          ...
          <path
            ...
            strokeDasharray={1}
            strokeDashoffset={1}
            filter="url(#splash-icon-glow)"
            style={{
              animation: 'splashDrawPath 1.5s cubic-bezier(0.4, 0, 0.2, 1) forwards',
            }}
          />
          {/* White fill — fades in after draw */}
          <path
      ...
      <div
        style={{
          marginTop: 6,
          filter: iconDone ? 'drop-shadow(0 0 6px rgba(149,218,248,0.4)) drop-shadow(0 0 16px rgba(36,84,91,0.6))' : 'none',
        ...
        >
          ...
          {DISPLAY_LETTERS.map((d, i) => (
            <path
              ...
              filter="url(#splash-text-glow)"
              style={iconDone ? {
                animation: `splashDrawPath 0.6s cubic-bezier(0.4, 0, 0.2, 1) ${i * 0.1}s both`,
              } : {
                strokeDashoffset: 1,
              }}
            />
      ...
      <div
        style={{
          ...
          width: textDone ? '100px' : '0px',
          transition: 'width 500ms cubic-bezier(0.4, 0, 0.6, 1)',
        }}
      />
